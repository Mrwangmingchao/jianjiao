...
export default {
  ...
  mounted () {
    axios({
      url: `/pages/category/${this.$store.state.id}?currentPage=1&sort=onShelfTime&order=desc&_=1562816202423`
    }).then(res => {
  ...
  methods: {
    ...
    loadMore () {
      ...
      this.index++
      axios({
        url: `/pages/category/${this.$store.state.id}?currentPage=${this.index}&sort=onShelfTime&order=desc&_=1562816202423`
      }).then(res => {
  ...
  watch: {
    id (old, nw) {
      axios({
        url: `/pages/category/${old}?currentPage=1&sort=onShelfTime&order=desc&_=1562816202423`
      }).then(res => {
        this.datalist = res.data.data
      })
    }
